...
- **The capture card now works on VPUs that are behind on Windows updates, and says so.** Units that never received the Windows cumulative update carry the original October 2018 packet monitor, which can count traffic but cannot record it. On those units the card now reports the traffic totals it can measure, leaves resets, retransmissions and drops blank rather than showing a misleading 0, and tells you the unit needs the Windows update — naming the build it is on. Previously the whole card just errored.
- **Traceroute now shows real hop timings.** Every intermediate hop reported "0 ms" regardless of the actual latency, which hid exactly the slow hop a traceroute is run to find. Each hop is now timed properly.
- A nearly-full recording drive (D:) is now **Worth knowing**, not a risk. The live stream is processed on C: and nothing in the broadcast path touches D:, so a full D: cannot stop or degrade a game going out - a unit with only that problem now reads **PASS**. It still says what it costs: the row reads "Drive D: almost full - recordings may not save", and the Disks tab still shows any drive over 90% as Critical with Storage Cleanup, so the urgency lives where the fix is. A full **C:** is unchanged and still stops the game.
- A nearly-full recording drive no longer reads **Stops tonight's game**. A 91%-full D: is a risk, not a blocker - it will not stop an event - and the Dashboard was calling it one. It happened whenever a finding had no readiness classification of its own: the display fell back to the collector's severity, and a collector saying "critical" only means "serious", never "this stops the game". Only the readiness policy decides that, and now only it can say so.
- "OCR" is gone from the screen everywhere it was still showing. The scoreboard camera now reads **Scoreboard** on Camera Connectivity, **Scoreboard camera** on Camera Calibrations, and **Role: Scoreboard** on Camera Hardware (the 1 Gbps variant says "Scoreboard (1 Gbps)", so a 100 Mbps link on that model still reads as the problem it is). The Camera Hardware subtitle no longer leads with "Full CGI probe". The camera's own serial number is left alone - that is stamped on the hardware.
- The Help page no longer sends you to the wrong tab for the Agent restart. It said the **Restart Agent + Coordinator** button was on **Pixellot Software** in the same sentence that correctly named **Service Status**; the button is on Service Status.
- **Copy for ticket** now reports the same thing the screen does. Three problems, all fixed: it could copy a different readiness verdict than the card was showing; it counted problems in the header that it never listed (a unit with a pegged CPU, full memory and a 93C temperature copied as "4 risks tonight" over a single camera-cable line, with the three serious ones named nowhere); and "Checked:" was the time you clicked, not the time Pulse read the unit, so a page left open for twenty minutes claimed a fresh check. The text now carries the real collection time with a timezone, the unit's serial number for RMAs, and a clear "DEMO DATA" banner when Pulse is running on demo data so it can't be mistaken for a real reading.
- The Copy for ticket button now tells you when a copy fails instead of doing nothing. It also works when Pulse is opened by IP address rather than localhost, where the browser blocks the normal copy method.
- Windows **Critical** events now show a red "Critical" chip. They were displaying as a blue chip labelled "Information" - the most severe class of Windows event shown as the least severe one. A Kernel-Power 41 ("rebooted without cleanly shutting down") read as routine information. The Windows Events sidebar badge also counts them now, so a VPU whose only recent entries are Critical no longer shows a clean sidebar.
- A failing drive now shows a red badge on the Disks tab. A SMART **Unhealthy** status rendered grey there while the Dashboard was already calling the same drive critical - two screens, opposite verdicts. A **Paused** Windows service now shows amber instead of grey.
...
### Removed
- The **SportzCast scoreboard** port test is gone from the Network tab. Nothing on the VPU requires TCP/1400-1405, so six tiles that could never fail readiness made up almost the whole Optional section and read as six more ports to understand. Scoreboard health is covered properly on the **ScoreConnect** tab, which tests the bot connection and the live feed. `sportzcast.net` is gone from Service Reachability too, so Pulse no longer probes SportzCast anywhere.
...
- Status callouts across the app now read as one consistent style. Warnings, findings, verdicts, service tiles and result panels used a thick coloured bar down their left edge; they now carry a thin outline in the same colour, so a red finding on one tab looks like a red finding on every other. The ping results on the Network tab also gained a Pass/Warn/Fail label, so their result no longer depends on spotting a coloured edge.
- A full recording drive is no longer reported twice. The Dashboard raised "Drive D: almost full" and Stream Readiness separately counted the same drive as a risk, so one problem appeared as two and the two disagreed about how serious it was. There is now one entry, at the severity readiness actually assigned it.
- Plain words in place of jargon across the app, and a verdict above the raw numbers. The Dashboard NIC row now says **Slow** instead of **Error** for a port that is up but running at 100 Mbps (the same state Camera Connectivity called "Degraded" - it says Slow there too now). The scoreboard camera is labelled **Scoreboard** instead of **OCR**, which was never spelled out anywhere. Camera detail badges read **Answering** / **Not answering** instead of **CGI** / **No CGI**. Calibration rows say **Picture-in-picture layout** and **Scoreboard regions** rather than the filenames Pulse looks for. Time Sync says **Clock source quality** with a note that lower is better, instead of a bare **Stratum** number. ScoreConnect says **Scoreboard feed reaching the VPU: Yes/No** instead of **Local Stream: Detected**. The Fault Isolator says "camera" instead of "CHU", and its unconfirmed verdict now reads **Probably the camera - swap test not finished**.
- Every gauge now says whether its number is **Normal**, **Running high** or **Too high**. The rings always changed colour at those thresholds, but nothing on screen said what the threshold was, so 63% looked the same as fine.
- Live Network Health leads with a sentence - "Traffic is flowing normally. Nothing is being re-sent." - above its counters, and its table headers name what they measure ("Waiting to send", "Download") with the normal range underneath.
- Each camera on Camera Hardware now carries a one-line summary of what Pulse actually checked, above its ~30 fields.
- Screen-reader and colour-blind support: the sidebar's warning triangles now carry the same words the Dashboard uses, so the severity no longer depends on telling red from amber. Status badges, icon-only buttons and the page title all have proper labels.
- The sidebar's section labels (TRIAGE, TROUBLESHOOTING...) are readable in light mode. They were the last text in either theme below the accessibility contrast standard.
- Dashboard findings now open in place to show what to do about them. Clicking a finding used to throw you onto another tab; the fix Pulse had already written for it only appeared once you got there. Click one now and the full recommendation drops down underneath it, with an **Open <tab>** link if you still want the detail. Nothing new was written - this is the same text that was always one tab away.
- New **Copy for ticket** button on the Findings card. It copies the unit name, the readiness verdict and every finding with its recommendation, formatted and ready to paste into a ticket, so you don't retype what's already on screen.
- Port Connectivity on the Network tab now leads with what each connection is *for* instead of its port number. The tiles used to read 53, 123, 443, 443, 1935, 2088 - two of them the same number with opposite results - and the three streaming paths were indistinguishable. They now read **Live video - main path**, **Live video - backup path**, **Live video - last resort**, **Clock sync**, **Name lookup (DNS)**, **Cloud services** and **SportzCast scoreboard**, with the protocol and port underneath. The three streaming paths also sit together in the order the stream actually tries them, so you can see at a glance which way the game is going out. The summary line now says "The venue is blocking 2 of the 10 connections this VPU needs" instead of "2 of 10 required blocked".
- The Dashboard now says how bad each problem is in plain words instead of severity codes. Findings read **Stops tonight's game**, **Risk tonight**, or **Worth knowing** in place of `[CRITICAL]` and `[WARNING]`, and the Stream Readiness card says "Nothing is stopping tonight's game - 3 risks tonight" instead of "0 blockers - 2 risks". The card and the list beside it could previously disagree - one saying no blockers while the other said CRITICAL - because they used two different scales. They now use one, and a problem can never be shown as less serious than the check that raised it. Also: "9 panels checked" now reads "Checked 9 areas", and the internal `policy v1` stamp is off the card.
...
- When both the gateway and the DNS server ignore ping but traffic and name resolution are working, the Network tab now says so once - "Ping (ICMP) is blocked on this network" - instead of two separate notes a tech had to piece together. Nothing about the unit changed; it's the same all-clear, stated as one conclusion so nobody keeps ping-testing a VPU whose pings are never coming back.
